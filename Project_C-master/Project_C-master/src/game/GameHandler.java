...
public class GameHandler extends JPanel implements Runnable {
    ...
    Block block4 = new Block(6100,750,50,50,"Crack");
    Block block5 = new Block(5850,700,50,50,"Is");
    Block block6 = new Block(6100,250,50,50,"Fix");
    ...
    public GameHandler() {
        ...
        cracks.add(new Crack(200 + 5500, 200, 50,50));
        cracks.add(new Crack(200 + 5500, 250, 50,50));
        cracks.add(new Crack(200 + 5500, 300, 50,50));
        cracks.add(new Crack(200 + 5500, 350, 50,50));
        cracks.add(new Crack(200 + 5500, 400, 50,50));
        cracks.add(new Crack(200 + 5500, 200, 50,50));
        cracks.add(new Crack(200 + 5500, 200, 50,50));
        cracks.add(new Crack(200 + 5500, 200, 50,50));
...
//        blocks.add(new Block(700,800,50,50,"IN"));
    ...
    private void update() {
        ...
        for (Staircase stair : staircases) {
            if (stair.isPlayerOnStair(player) && stair.active) {
                stair.active = false;

                // Example: teleport player to next level
                player.x = 3750; // ← set this to wherever level 4 begins
                player.y = 400;

                nextLevel4 = true;

                break; // Stop checking once one staircase is triggered
            }
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (nextLevel3) {
            ...
            staircases.add(new Staircase(2150, 150, 50, 50));
            nextLevel3 = false;
        }
        ...
        g.setColor(Color.PINK); // Or whatever color you want
        for (Staircase stair : staircases) {
            if (stair.active) {
    ...
    private void updateRules() {
        ...
        for (Block block : blocks) {
            ...
            if (foundNextLevel2 && staircases.isEmpty()) {
               // staircases.add(new Staircase(2150, 150, 50, 50)); // Adjust coordinates to your level
            }
...
class Staircase {
    ...
    public boolean isPlayerOnStair(Player player) {
        return active &&
                player.x + player.size > x && player.x < x + width &&
                player.y + player.size > y && player.y < y + height;
    }
}
...
class Crack {
    ...
    public Crack(int x, int y, int width, int height) {
        ...
        this.width = width;
        this.height = height;
        this.fixed = false; // By default, the crack is broken (not fixed)
    }
    ...
    public boolean isPassable() {
        return fixed; // Only passable if it's fixed
    }

    public void draw(Graphics g) {
        g.setColor(fixed ? Color.BLUE : Color.BLACK); // Blue if fixed, black if broken
        g.fillRect(x, y, width, height);
    }
